...
ReIP=10.7.13.53
password=sonicwall
#time=1000     #Wait $time seconds to exit the terminal.
##clear the old data
cd /etc/ssh
./if-eth_down.sh
rm dpi-ssh_*
...
do
	...
	ssh -b 172.20.$a.$b -X sonicwall@$ReIP &
	...
	#echo "spawn ssh -b 172.20.$a.$b -X sonicwall@$ReIP">>/etc/ssh/dpi-ssh_$i.sh
	...
	#echo "Opened terminal_$i successfully"
	#sleep 1
	let b++
done
